...
//新增
function lunboData(){
    let $name = $('.mask .addwin form input ').val()
    //判断数据
    if($img.src==''|| $name==''){
        alert('请输入对应数据')
        return
  ...
  $('table').bootstrapTable({
    ...
    columns: [
        ...
        ,{
            ...
            align:'center',
            formatter:function(value,row,index){
                return `<img src="../../public/images/friendlink/${row.img}">`
            }
    },{
        field:'name',
        title:'赞助商',
...
function modify(e,index){
    ...
    let $sname = $('.maskTwo .addwin input');
    $('#myModalT').modal('show');
    $("#modifys").attr("data-bs-target","#myModalT");
    $img.prop('src',`../../public/images/friendlink/${e.img}`);
    $sname.val(e.name)
    //图片回显
    $file.change( function (event) {
        let file = event.target.files[0];
      ...
      //获得修改点击按钮
      $('#xiugai').click(()=>{
        $sname=$('.maskTwo .addwin input').val();
         //整合数据
      json={
        img:imgName,
        name:$sname,
