...
//新增
function lunboData(){
    let title = document.querySelectorAll('.mask input[type=text]')[0].value;
    let arr = document.querySelector('.mask select').value.split(',');
    let content =document.querySelector('.mask textarea').value
    let herf = arr[0];
    let text = arr[1]
    //判断数据
    if(img.src==''|| title==='' ||content==''){
        alert('请输入对应数据')
        return
    }
    //整合传递数据
...
function modify(e,index){
    ...
    let title = document.querySelectorAll('.maskTwo input[type=text]')[0]
    let content = document.querySelector('.maskTwo textarea')
    $('#myModalT').modal('show');
    ...
    file.addEventListener("change", function (event) {
     ...
     $('#xiugai').click(()=>{
        ...
        let arr = document.querySelector(' .maskTwo  select').value.split(',');
        let content = document.querySelector('.maskTwo textarea').value
        let herf = arr[0];
        let text = arr[1]
