...
//   新增
function lunboData(){
    let $title = $('.mask input[type=text]').eq(0).val();
    let $arr = $('.mask select').val().split(',');
    let $content = $('.mask textarea').val()
    let herf = $arr[0];
    let text = $arr[1];
    //判断数据
    if($img.src==''|| $title==='' ){
...
function modify(e,index){
    let imgName
    let $file = $('.maskTwo .addwin input[type=file]');
    let $img = $('.maskTwo .addwin img');
    let $title = $('.maskTwo input[type=text]').eq(0)
    let $content = $('.maskTwo textarea')
    $('#myModalT').modal('show');
    $("#modifys").attr("data-bs-target","#myModalT");
    $img.prop('src',`../../public/images/lunbo/${e.img}`);
    $title.val(e.title)
    ...
    $file.change( function (event) {
      ...
      //获得修改点击按钮
      $('.maskTwo .addwin form button:nth-of-type(2)').onclick(()=>{
        let $title = $('.maskTwo input[type=text]').eq(0).val()
        let $arr = $(' .maskTwo  select').val().split(',');
        let $content = $('.maskTwo textarea').val()
        let herf = $arr[0];
        let text = $arr[1]
