...
//新增
function lunboData(){
    let $Type = $('.addwin form input').eq(0).val()
    let $TypeID = $('.addwin form input').eq(1).val()
    //判断数据
    if($Type==''||$TypeID==''){
        alert('请输入对应数据')
        return
...
function modify(e,index){
    let $Type = $('.maskTwo .addwin form input').eq(0)
    let $TypeID = $('.maskTwo .addwin form input').eq(1)
    $('#myModalT').modal('show');
